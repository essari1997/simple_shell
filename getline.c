	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			rm_comments(*buf);
			build_h(info, *buf, info->histcount++);
